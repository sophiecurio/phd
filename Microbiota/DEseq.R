...
png(file = paste("R/DESeq2/", paste(name, '_sig.png'), sep = ''), units="in", width=5, height=5, res=300)
...
file = paste("R/DESeq2/", paste(name, '.csv'), sep ='')
write.csv(sigtab, file)

#volcao plot
png(file = paste("R/DESeq2/", paste(name, '_volcano.png'), sep = ''), units="in", width=5, height=5, res=300)
plotMA(res)
dev.off()
...
d  <- plotCounts(phy_deseq, gene='e6916a3a5113051772ad033df382f89e', intgroup="Genotype", returnData=TRUE)
ggplot(d, aes(x = Genotype, y = count)) + 
  geom_point(position=position_jitter(w = 0.1,h = 0)) +
  geom_text_repel(aes(label = rownames(d))) + 
  theme_bw() +
  ggtitle('e6916a3a5113051772ad033df382f89e') +
